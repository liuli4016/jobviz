...
//var e = document.getElementById("dataselection");
//var select_year = e.options[e.selectedIndex].text;


var select_year = "";

if (!localStorage.getItem("chosenyear"))
{
   var e = document.getElementById("dataselection");
   select_year = e.options[e.selectedIndex].value;
}

else
{
   select_year = localStorage.getItem("chosenyear");
   var element = document.getElementById('dataselection');
   element.value = select_year;
}

//var csvfilename = "jobs2018.csv";
//var jsonfilename = "jobs2018.json";

var csvfilename = "Jobs" + select_year + ".csv";
var jsonfilename = "Jobs" + select_year + ".json";
...
d3.text(csvfilename, function (unparsedData) 
{
   csvdata = d3.csv.parseRows(unparsedData);  
         
   for (var scan = 1; scan < csvdata.length; scan++) 
       jobdsp.push(csvdata[scan][0]);                                                                                                                                                                                                                                                     
}); 
...
var subgroupsign = 0;
var currentDbcText = "Job Postings: " + select_year;
...
var alljobnodes = [];
var alljobnodes_volunteer = [], alljobnodes_internship = [], alljobnodes_ftentrylevel = [], alljobnodes_ftexperienced = [];          	       	
var alljobnodes_freshman = [], alljobnodes_sophomore = [], alljobnodes_junior = [], alljobnodes_senior = [], alljobnodes_graduate = [], alljobnodes_alumni = [], alljobnodes_unspecified = [];
...
function refreshwindow()
{
    ...
    var yvalue = document.getElementById("dataselection");
    var currentyear = yvalue.options[yvalue.selectedIndex].value;    
    ...
    .attr('stroke-width', function (l)
	  {		  	
		      ...
		      if (thiscoef < 0.05)
				  ...
				  else if (thiscoef >= 0.05 && thiscoef < 0.15)
				     thickness = 8;
				      	      
				  else if (thiscoef >= 0.15 && thiscoef < 0.25) 
				     thickness = 16;  
				      	   	  
				  else if (thiscoef >= 0.25 && thiscoef < 0.4) 
				     thickness = 20;
				      	   	  
				  else if (thiscoef >= 0.4) 
		  	   	  thickness = 30;	       
...
{	
    var gapangle = majorarc_angle/151;    
	         ...
	         {               		            	  	
                 var deep;                                                 
		             ...
		             if (majorplotmode == 1)
		             {
		             	   ...
		             	   {
		             	  	  d.x = jobarc_angle + gapangle*(school_SAS.indexOf(d.key));
		             	  	  var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		             	  	  var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		             	  	  did = part1 + "SAS" + part3;	
		             	      schoolandmajor[0].push(d);	 
		             	      majorgroupIDByIndex.set(d.name-alljobnodes.length, 0);            	  	 
		             	   }
		             	  
		             	   else if (school_SEBS.indexOf(d.key) > -1)
		             	   {
		             	  	  d.x = jobarc_angle + gapangle*(school_SEBS.indexOf(d.key)+school_SAS.length);
		             	  	  var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		             	  	  var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		             	  	  did = part1 + "SEBS" + part3;
		             	  	  schoolandmajor[1].push(d);
		             	  	  majorgroupIDByIndex.set(d.name-alljobnodes.length, 1); 
		             	   }   
		             	  
		             	   else if (school_MGSA.indexOf(d.key) > -1)
		             	   {
		             	 	    d.x = jobarc_angle + gapangle*(school_MGSA.indexOf(d.key)+school_SAS.length+school_SEBS.length);
		             	 	    var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		               	    var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		            	 	    did = part1 + "MGSA" + part3;
		             	 	    schoolandmajor[2].push(d);
		               	    majorgroupIDByIndex.set(d.name-alljobnodes.length, 2); 
		            	   }
		             	  
		             	   else if (school_EMSP.indexOf(d.key) > -1)
		             	   {
		             	  	  d.x = jobarc_angle + gapangle*(school_EMSP.indexOf(d.key)+school_SAS.length+school_SEBS.length+school_MGSA.length);
		             	  	  var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		             	  	  var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		             	  	  did = part1 + "EMSP" + part3;
		             	  	  schoolandmajor[3].push(d);
		             	  	  majorgroupIDByIndex.set(d.name-alljobnodes.length, 3); 
		             	   }
		             	  
		             	   else if (school_RBS.indexOf(d.key) > -1)
		             	   {
		             	  	  d.x = jobarc_angle + gapangle*(school_RBS.indexOf(d.key)+school_SAS.length+school_SEBS.length+school_MGSA.length+school_EMSP.length);
		             	  	  var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		             	  	  var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		             	  	  did = part1 + "RBS" + part3;
		             	  	  schoolandmajor[4].push(d);
		             	  	  majorgroupIDByIndex.set(d.name-alljobnodes.length, 4); 
		             	   }
		             	  
		             	   else if (school_SCI.indexOf(d.key) > -1)
		             	   {
		             	  	  d.x = jobarc_angle + gapangle*(school_SCI.indexOf(d.key)+school_SAS.length+school_SEBS.length+school_MGSA.length+school_EMSP.length+school_RBS.length);
		             	  	  var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		             	  	  var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		             	  	  did = part1 + "SCI" + part3;
		             	  	  schoolandmajor[5].push(d);
		             	  	  majorgroupIDByIndex.set(d.name-alljobnodes.length, 5); 
		             	   }
		             	  
		             	   else if (school_SOE.indexOf(d.key) > -1)
		             	   {
		              	    d.x = jobarc_angle + gapangle*(school_SOE.indexOf(d.key)+school_SAS.length+school_SEBS.length+school_MGSA.length+school_EMSP.length+school_RBS.length+school_SCI.length);
		           	  	    var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		           	  	    var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		           	  	    did = part1 + "SOE" + part3;
		           	  	    schoolandmajor[6].push(d);
		           	  	    majorgroupIDByIndex.set(d.name-alljobnodes.length, 6); 
		           	     }
		             	  
		             	   else if (school_EBSPPP.indexOf(d.key) > -1)
		             	   {
		             	  	  d.x = jobarc_angle + gapangle*(school_EBSPPP.indexOf(d.key)+school_SAS.length+school_SEBS.length+school_MGSA.length+school_EMSP.length+school_RBS.length+school_SCI.length+school_SOE.length);
		             	  	  var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		             	  	  var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		             	  	  did = part1 + "EBSPPP" + part3;
		             	  	  schoolandmajor[7].push(d);
		             	  	  majorgroupIDByIndex.set(d.name-alljobnodes.length, 7); 
		             	   }
		             	  
		             	   else if (school_SMLR.indexOf(d.key) > -1)
		             	   {
		             	  	  d.x = jobarc_angle + gapangle*(school_SMLR.indexOf(d.key)+school_SAS.length+school_SEBS.length+school_MGSA.length+school_EMSP.length+school_RBS.length+school_SCI.length+school_SOE.length+school_EBSPPP.length);
		             	  	  var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		             	  	  var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		             	  	  did = part1 + "SMLR" + part3;
		             	  	  schoolandmajor[8].push(d);
		             	  	  majorgroupIDByIndex.set(d.name-alljobnodes.length, 8); 
		             	   }
		             	  
		             	   else if (school_SSW.indexOf(d.key) > -1)
		             	   {
		             	  	  d.x = jobarc_angle + gapangle*(school_SSW.indexOf(d.key)+school_SAS.length+school_SEBS.length+school_MGSA.length+school_EMSP.length+school_RBS.length+school_SCI.length+school_SOE.length+school_EBSPPP.length+school_SMLR.length);
		             	  	  var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		             	  	  var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		             	  	  did = part1 + "SSW" + part3;
		             	  	  schoolandmajor[9].push(d);
		             	  	  majorgroupIDByIndex.set(d.name-alljobnodes.length, 9); 
		             	   }
		             	  
		             	   else if (school_Other.indexOf(d.key) > -1)
		             	   {
		             	  	  d.x = jobarc_angle + gapangle*(school_Other.indexOf(d.key)+school_SAS.length+school_SEBS.length+school_MGSA.length+school_EMSP.length+school_RBS.length+school_SCI.length+school_SOE.length+school_EBSPPP.length+school_SMLR.length+school_SSW.length);
		             	  	  var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		             	  	  var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		             	      did = part1 + "Other Majors" + part3;
		             	     	schoolandmajor[10].push(d);
		             	     	majorgroupIDByIndex.set(d.name-alljobnodes.length, 10); 
		             	   }          	  	             	  	             	  
		             }
                 ...
                 else if (majorplotmode == 2)
		             {
		                 if (area_BAES.indexOf(d.key) > -1)
		             	   {
		             	  	  d.x = jobarc_angle + gapangle*(area_BAES.indexOf(d.key));
		             	  	  var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		             	  	  var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		             	   ...
		             	   {
		             	  	  d.x = jobarc_angle + gapangle*(area_Physical.indexOf(d.key)+area_BAES.length);
		             	  	  var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		             	  	  var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		             	   ...
		             	   {
		             	  	  d.x = jobarc_angle + gapangle*(area_CMS.indexOf(d.key)+area_BAES.length+area_Physical.length);
		             	  	  var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		             	  	  var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		             	   ...
		             	   {
		             	  	  d.x = jobarc_angle + gapangle*(area_Engineering.indexOf(d.key)+area_BAES.length+area_Physical.length+area_CMS.length);
		             	  	  var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		             	      var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		             	   ...
		             	   {
		              	    d.x = jobarc_angle + gapangle*(area_Business.indexOf(d.key)+area_BAES.length+area_Physical.length+area_CMS.length+area_Engineering.length);
		             	 	    var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		               	    var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		             	   ...
		             	   {
		             	 	    d.x = jobarc_angle + gapangle*(area_Communications.indexOf(d.key)+area_BAES.length+area_Physical.length+area_CMS.length+area_Engineering.length+area_Business.length);
		               	    var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		          	  	    var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		             	   ...
		             	   {
		              	    d.x = jobarc_angle + gapangle*(area_Other.indexOf(d.key)+area_BAES.length+area_Physical.length+area_CMS.length+area_Engineering.length+area_Business.length+area_Communications.length);
		             		    var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		             	 	    var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		           	     ...
		           	     {
		           	  	    d.x = jobarc_angle + gapangle*(area_Social.indexOf(d.key)+area_BAES.length+area_Physical.length+area_CMS.length+area_Engineering.length+area_Business.length+area_Communications.length+area_Other.length);
		           	  	    var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		           	  	    var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		             	   ...
		             	   {
		             	 	    d.x = jobarc_angle + gapangle*(area_Education.indexOf(d.key)+area_BAES.length+area_Physical.length+area_CMS.length+area_Engineering.length+area_Business.length+area_Communications.length+area_Other.length+area_Social.length);
		               	    var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		           	  	    var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		             	   ...
		             	   {
		             	 	    d.x = jobarc_angle + gapangle*(area_Arts.indexOf(d.key)+area_BAES.length+area_Physical.length+area_CMS.length+area_Engineering.length+area_Business.length+area_Communications.length+area_Other.length+area_Social.length+area_Education.length);
		             	  	  var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		             	  	  var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		           	     ...
		           	     {
		           	  	    d.x = jobarc_angle + gapangle*(area_LAH.indexOf(d.key)+area_BAES.length+area_Physical.length+area_CMS.length+area_Engineering.length+area_Business.length+area_Communications.length+area_Other.length+area_Social.length+area_Education.length+area_Arts.length);
		           	  	    var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		           	  	    var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		             	   ...
		             	   {
		             	 	    d.x = jobarc_angle + gapangle*(area_LL.indexOf(d.key)+area_BAES.length+area_Physical.length+area_CMS.length+area_Engineering.length+area_Business.length+area_Communications.length+area_Other.length+area_Social.length+area_Education.length+area_Arts.length+area_LAH.length);
		               	    var part1 = d.id.substring(0, d.id.indexOf("@")+1);
		           	  	    var part3 = d.id.substring(d.id.indexOf("|"), d.id.length);
		           	  	    did = part1 + "Lit & Lang Majors" + part3;
		           	  	    areaandmajor[11].push(d);
		             	  	  majorgroupIDByIndex.set(d.name-alljobnodes.length, 11); 
		             	   }		             	  		             	    		             	  		            	    
		             }
              ...
              .text(function(d) 
              {             	  
              	   majorIDByIndex.set(d.name, d.name-alljobnodes.length);  		
